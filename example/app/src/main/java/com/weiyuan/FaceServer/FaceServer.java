package com.weiyuan.FaceServer;

import android.content.Context;
import android.graphics.Rect;
import android.util.Log;

import com.weiyuan.model.FaceRegisterInfo;
...
public class FaceServer {
    ...
    public boolean init(Context context) {
        synchronized (this) {
            if (faceEngine == null && context != null) {
                ...
                int engineCode = faceEngine.init(DetectMode.SDK_DETECT_MODE_IMAGE, RotateDegree.SDK_Rotate_0, 720, FaceEngine.SDK_FACE_RECOGNITION | FaceEngine.SDK_FACE_DETECT);
                if (engineCode == ErrorInfo.OK) {
    ...
    public boolean registerNv21(Context context, byte[] nv21, int width, int height, FaceInfo faceInfo, String name) {
        synchronized (this) {
            ...
            if (faceEngine == null || context == null || nv21 == null  || nv21.length != width * height * 3 / 2) {
            ...
            Log.i(TAG, "save image path:" + imgDir.getAbsolutePath());
            FaceFeature faceFeature = new FaceFeature();

            //特征提取
            int code = faceEngine.getFeature(nv21, width, height, SdkImageFormat.NV21, faceInfo, faceFeature);
            if (code != ErrorInfo.OK) {
                ...
                try {
                    ...
                    String file = imgDir + File.separator + userName + IMG_SUFFIX;

                    // 保存头像
                    saveHeadImage(nv21, width, height, faceInfo.getOrient(), cropRect, SdkImageFormat.NV21, file);
    ...
    public boolean registerBgr24(Context context, byte[] bgr24, int width, int height, String name) {
        synchronized (this) {
            ...
            if (faceEngine == null || context == null || bgr24 == null || bgr24.length != width * height * 3) {
            ...
            int code = faceEngine.detectFaces(bgr24, width, height, SdkImageFormat.BGR24, faceInfoList);
            if (code == ErrorInfo.OK && faceInfoList.size() > 0) {
                FaceFeature faceFeature = new FaceFeature();

                //特征提取
                code = faceEngine.getFeature(bgr24, width, height, SdkImageFormat.BGR24, faceInfoList.get(0), faceFeature);
                String userName = name == null ? String.valueOf(System.currentTimeMillis()) : name;
                try {
                    //保存注册结果（注册图、特征数据）
                    if (code == ErrorInfo.OK) {
                        ...
                        // 保存头像
                        saveHeadImage(bgr24, width, height, faceInfoList.get(0).getOrient(), cropRect, SdkImageFormat.BGR24, file);
     ...
     * @return 头像的图像数据
     */
    private void saveHeadImage(byte[] originImageData, int width, int height, int orient, Rect cropRect, int imageFormat, String fileName) {
        byte[] headImageData = SdkImageUtil.createImageData(cropRect.width(), cropRect.height(), SdkImageFormat.BGR24);
        int cropCode = SdkImageUtil.cropImage(originImageData, headImageData, width, height, cropRect, imageFormat);
        if (cropCode != SdkImageUtilError.CODE_SUCCESS) {
            throw new RuntimeException("crop image failed, code is " + cropCode);
        }

        // 将创建一个Bitmap，并将图像数据存放到Bitmap中
        if (SdkImageUtil.imageDataToFile(headImageData, cropRect.width(), cropRect.height(), SdkImageFormat.BGR24, orient, fileName) != SdkImageUtilError.CODE_SUCCESS) {
