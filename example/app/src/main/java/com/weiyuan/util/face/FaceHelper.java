package com.weiyuan.util.face;

import android.hardware.Camera;
import android.util.Log;

import com.weiyuan.model.FacePreviewInfo;
import com.weiyuan.util.TrackUtil;
    ...
    public List<FacePreviewInfo> onPreviewFrame(byte[] nv21) {
        if (faceListener != null) {
            if (ftEngine != null) {
                ...
                int code = ftEngine.detectFaces(nv21, previewSize.width, previewSize.height, SdkImageFormat.NV21, faceInfoList);
            ...
            if (faceListener != null && nv21Data != null) {
                if (frEngine != null) {
                    ...
                    synchronized (frEngine) {
                        frCode = frEngine.getFeature(nv21Data, width, height, format, faceInfo, faceFeature);
                    }
                    if (frCode == ErrorInfo.OK) {
            ...
            if (faceListener != null && nv21Data != null) {
                if (flEngine != null) {
                    ...
                    synchronized (flEngine) {
                        flCode = flEngine.setFaces(nv21Data, width, height, format, Arrays.asList(faceInfo));
                    }
                    flCode = flEngine.getLiveness(livenessInfoList);

                    if (flCode == ErrorInfo.OK && livenessInfoList.size() > 0) {
