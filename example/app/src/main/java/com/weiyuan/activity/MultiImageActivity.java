...
import com.weiyuan.model.ItemShowInfo;
import com.weiyuan.widget.MultiFaceInfoAdapter;
...
public class MultiImageActivity extends BaseActivity {
    ...
    private void initEngine() {
        ...
        faceEngineCode = faceEngine.init( DetectMode.SDK_DETECT_MODE_IMAGE, RotateDegree.SDK_Rotate_0,
                1080, FaceEngine.SDK_FACE_RECOGNITION | FaceEngine.SDK_AGE | FaceEngine.SDK_FACE_DETECT | FaceEngine.SDK_GENDER  | FaceEngine.SDK_FACE3DANGLE);
    ...
    public void processImage(Bitmap bitmap, int type) {
        ...
        bitmap = SdkImageUtil.getAlignedBitmap(bitmap, true);
...
//        Log.i(TAG, "processImage:bitmapToBgr24 cost =  " + (System.currentTimeMillis() - start));
        ...
        if (bgr24 != null) {
            ...
            int faceFeatureCode = faceEngine.getFeature(faceFeatureList);

            if ((ageCode | genderCode | faceFeatureCode) != ErrorInfo.OK) {
            ...
            if (faceInfoList.size() > 0) {
                if (type == TYPE_MAIN) {
                    ...
                    mainFeature = faceFeatureList.get(0);
